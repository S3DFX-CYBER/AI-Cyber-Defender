...
class TestEndToEndFlow:
    ...
    def test_ingest_health(self, _headers):
    ...
    def test_analyzer_health(self, _headers):
    ...
    def test_event_retrieval(self, headers):
        ...
        # Retrieve the event
        get_response = requests.get(f"{INGEST_URL}/v1/events/{event_id}", headers=headers)
    ...
    def test_authentication_required(self):
        ...
        response = requests.post(
            f"{INGEST_URL}/v1/events/llm",
            json={"source_type": "test", "source_id": "test", "model": "gpt-4", "prompt": "Test"},
        )

        # Should fail without API key
        assert response.status_code in [401, 422]

    def test_invalid_api_key_rejected(self, _headers):
