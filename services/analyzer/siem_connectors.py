...
class SIEMDispatcher:
    ...
    def __init__(self) -> None:
        ...
        self.include_sensitive = os.getenv("INCLUDE_SENSITIVE_SIEM_FIELDS", "false").lower() == "true"
    ...
    def _sanitize_event(self, event: Dict[str, Any]) -> Dict[str, Any]:
        ...
        redacted = dict(event)
        for field in ("prompt", "response_text", "context", "agent_trace"):
    ...
    async def publish(self, event: Dict[str, Any]) -> None:
        ...
        if self.splunk_url and self.splunk_token:
            tasks.append(self._post_splunk(safe_event))
        if self.sentinel_url:
            tasks.append(self._post_sentinel(safe_event))
        if self.elastic_url:
            tasks.append(self._post_elastic(safe_event))
