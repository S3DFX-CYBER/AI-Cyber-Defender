...
def load_dataset(data_path: str) -> tuple[list, list]:
    ...
    logger.info(
        f"Loaded {len(prompts)} samples: {sum(labels)} malicious, {len(labels) - sum(labels)} benign"
    )
...
def train_model(
    prompts: list, labels: list, model_type: str = "logistic", test_size: float = 0.2
) -> tuple:
    """
    ...
    vectorizer = TfidfVectorizer(
        max_features=5000, ngram_range=(1, 3), min_df=1, max_df=0.95, stop_words="english"
    )
...
def test_model(model_path: str, prompts: list | None = None):
...
def main():
    ...
    parser.add_argument(
        "--data", type=str, default=DEFAULT_DATA_PATH, help="Path to training data JSON file"
    )
    parser.add_argument(
        "--output", type=str, default=DEFAULT_MODEL_PATH, help="Path to save trained model"
    )
    parser.add_argument(
        "--model",
        type=str,
    ...
    parser.add_argument(
        "--test-only", action="store_true", help="Only test existing model, don't train"
    )
    ...
    if args.test_only:
        ...
        # Train model
        model, vectorizer, accuracy = train_model(prompts, labels, model_type=args.model)
